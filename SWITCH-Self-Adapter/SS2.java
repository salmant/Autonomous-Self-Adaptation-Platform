...
public class SS2 {
	public static void main(String[] args)  throws Exception {
		...
		try {
			...
			///////////////////
			if (AVG_CPU > 80){
				if ((AVG_CPU >= 80) && (AVG_CPU < 90)) {
					double inc_tem = 0.2 * containers; 
					if (inc_tem < 1) inc = 1;
					else inc = (int) Math.floor(inc_tem);
				} else if (AVG_CPU >= 90) {
							double inc_tem = 0.3 * containers; 
							if (inc_tem < 1) inc = 1;
							else inc = (int) Math.floor(inc_tem);
					   }
			...
			///////////////////////////////////
			if (AVG_CPU < 50){
				if ((AVG_CPU > 30) && (AVG_CPU <= 50)){
					double dec_tem = 0.2 * containers; 
					if (dec_tem < 1) dec = 1;
					else dec = (int) Math.floor(dec_tem);
				} else if (AVG_CPU <= 30) {
							double dec_tem = 0.3 * containers; 
							if (dec_tem < 1) dec = 1;
							else dec = (int) Math.floor(dec_tem);
					   }
