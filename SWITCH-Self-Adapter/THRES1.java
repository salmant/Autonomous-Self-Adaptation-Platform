	...
	public static void main(String[] args)  throws Exception {
		...
		try {
			...
			if ((AVG_CPU > CPU_threshold)|| (AVG_Mem > Mem_threshold)){
					int inc1 = 0; // the number of container to be added to the cluster based on CPU usage
					if (AVG_CPU > CPU_threshold) inc1 = 1;
					int inc2 = 0; // the number of container to be added to the cluster based on Memory usage
					if (AVG_Mem > Mem_threshold) inc2 = 1;
					inc = Math.max(inc1, inc2); // the number of container to be added to the cluster
					System.out.println("#inc: " + inc); 
					///////////////////
					int j=0;
					for (j=0; j<num_Servers; j++){
						if (server_IP[j][1].equals("FREE")){
							server_IP[j][1] = String.valueOf(containers+1); // "occupied" 
							Run_a_Container server = new Run_a_Container(server_IP, j, server_IP[j][0], subid, String.valueOf(containers+1));
							server.start();
