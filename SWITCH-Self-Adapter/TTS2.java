...
public class TTS2 {
	public static void main(String[] args)  throws Exception {
		...
		double percent_TTS2 = 0.2; // %20 - TTS can also adjust the number of instances by a percentage
		int num_Servers = 12;
		...
		try {
			...
			///////////////////
			int Tolerance = (int) Math.ceil(10 / containers);
			if (AVG_CPU > (CPU_threshold+Tolerance)){
				inc = (int) Math.ceil(percent_TTS2 * containers);
			...
			///////////////////////////////////
			if (AVG_CPU < (CPU_threshold-Tolerance)){
				if (((percent_TTS2 * containers)> 0) && ((percent_TTS2 * containers)< 1)) dec = 1; 
				else dec = (int) Math.floor(percent_TTS2 * containers);
			...
			Thread.sleep(Math.abs(60000 - (curtime-lasttime) ));
